...
def weighted_mse(input, target, abs_range, weight_val):
    #print(input[0][0])
    weight = torch.where(abs(target) < abs_range, 1, weight_val)
    #print(weight[0][0])
    
    return (weight*((input - target) ** 2)).mean()


def train(epochs, test_epochs, learning_rate, dataset_dir, batch_size, num_bins, loss_fn, optim, conv_transpose, abs_range, weight_val):
    ...
    for epoch in range(1,epochs+1):
        ...
        for data in trainloader:
            ...
            loss = weighted_mse(output, gt, abs_range, weight_val)
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            for data in testloader:
                ...
                loss = weighted_mse(test_output, test_gt, 2)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-a",  "--abs_range",       help="for weighted mse",            type=int,   default=1)
    parser.add_argument("-w",  "--weight_val",       help="for weighted mse",            type=int,   default=2)
